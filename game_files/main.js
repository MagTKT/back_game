...
/* Changes:
  ...
  1. Added the drawing calls for drawing the grass and ice_cubes in render.
  2. Added a p element for showing the number of ice_cubes collected.
...
window.addEventListener("load", function(event) {
  ...
  var render = function() {
    ...
    for (let index = game.world.ice_cubes.length - 1; index > -1; -- index) {

      let ice_cube = game.world.ice_cubes[index];

      frame = game.world.tile_set.frames[ice_cube.frame_value];

      display.drawObject(assets_manager.tile_set_image,
      frame.x, frame.y,
      ice_cube.x + Math.floor(ice_cube.width * 0.5 - frame.width * 0.5) + frame.offset_x,
      ice_cube.y + frame.offset_y, frame.width, frame.height);
  ...
  p.setAttribute("style", "color:#1054b8; font-size:2.0em; position:fixed; -webkit-text-stroke-width: 1px; -webkit-text-stroke-color: white; ");
  p.innerHTML = "Glace: 0";
  ...
  assets_manager.requestJSON(ZONE_PREFIX + game.world.zone_id + ZONE_SUFFIX, (zone) => {
    ...
    assets_manager.requestImage("pingoin_glacon.png", (image) => {
